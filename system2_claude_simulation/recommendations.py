...
class RecommendationRequest(BaseModel):
    ...
    @field_validator("current_occupancy", "competitor_availability")
    @classmethod
    def validate_ratio_01(cls, v: float) -> float:
        if v < 0 or v > 1:
    ...
    @field_validator("same_day_demand_score", "event_density", "neighborhood_availability")
    @classmethod
    def validate_optional_ratio_01(cls, v: float | None) -> float | None:
        if v is not None and (v < 0 or v > 1):
            raise ValueError("Optional ratio must be between 0 and 1")
        return v

    @field_validator("repurchase_probability", "churn_risk", "cancellation_rate")
    @classmethod
    def validate_probability(cls, v: float) -> float:
        if v < 0 or v > 1:
            raise ValueError("Probability-like value must be between 0 and 1")
        return v

    @field_validator("days_to_arrival")
    @classmethod
    def validate_days_to_arrival(cls, v: int) -> int:
        if v < 0 or v > 365:
            raise ValueError("days_to_arrival must be between 0 and 365")
        return v
