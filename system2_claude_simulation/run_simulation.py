...
def _push_metrics_snapshot(hour: int, avg_mare: float, avg_crm: float, avg_acq: float,
    ...
    msg = (
        ...
        f"| 模型 | 均价 | 覆盖 |\n"
        f"|------|------|------|\n"
        f"| MARE（2-3星） | MOP {avg_mare:.0f} | 145家 |\n"
        f"| DirectorAI CRM（2-3星） | MOP {avg_crm:.0f} | 145家 |\n"
        f"| SelfACQ 直销（4-5星） | MOP {avg_acq:.0f} | 280家 |\n"
        f"{market_section}\n"
...
def _push_daily_summary(summary: dict):
    ...
    msg = (
        ...
        f"**模型定价（日均）**\n"
        f"| 模型 | 均价 |\n|------|------|\n"
        f"| MARE（2-3星，145家） | MOP {summary['avg_mare_price']} |\n"
        f"| DirectorAI CRM（2-3星） | MOP {summary['avg_crm_price']} |\n"
        f"| SelfACQ 直销（4-5星，280家） | MOP {summary['avg_selfacq_offer']} |\n\n"
        f"**运行状态**\n"
...
def get_macau_market_signal(sim_hour: int, real_data: dict) -> dict:
    ...
    if mc_pace is not None and mc_source not in ("no_key", "makcorps_failed", "import_error"):
