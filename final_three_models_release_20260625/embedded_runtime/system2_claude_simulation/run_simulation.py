...
logger = logging.getLogger("insightbridge.system2")
...
try:
    ...
    logger.warning("HROS V6 fallback engaged; continuing on V5 path: %s", _v6_err)
    os.environ["INSIGHTBRIDGE_V6_FALLBACK_REASON"] = str(_v6_err)
...
def _is_infra_issue(issue: str) -> bool:
    text = (issue or "").lower()
    return any(keyword.lower() in text for keyword in _INFRA_KEYWORDS)


def _split_issue_channels(raw) -> tuple[list[str], list[str]]:
    pricing_issues: list[str] = []
    infra_issues: list[str] = []
    for issue in _normalize_anomalies(raw):
        if _is_infra_issue(issue):
            infra_issues.append(issue)
        else:
            pricing_issues.append(issue)
    return pricing_issues, infra_issues
...
def _summarize_hour_health(hour_results: list[tuple]) -> tuple[int, int, int]:
    ...
    for _, _, _, raw in hour_results:
        pricing_issues, infra_issues = _split_issue_channels(raw)
        if pricing_issues:
            anomalies += 1
        if any("EXCEPTION" in issue for issue in pricing_issues + infra_issues):
            failures += 1
...
def init_db():
    ...
    """)
    existing_cols = {row[1] for row in c.execute("PRAGMA table_info('hourly_runs')").fetchall()}
    if "pricing_anomaly" not in existing_cols:
        c.execute("ALTER TABLE hourly_runs ADD COLUMN pricing_anomaly TEXT DEFAULT ''")
    if "infra_error" not in existing_cols:
        c.execute("ALTER TABLE hourly_runs ADD COLUMN infra_error TEXT DEFAULT ''")
...
def write_daily_summary(conn: sqlite3.Connection, day: int):
    ...
    anomaly_count = c.execute(
        "SELECT COUNT(*) FROM hourly_runs WHERE pricing_anomaly != '' AND sim_hour BETWEEN ? AND ?",
        (start_hour, end_hour - 1),
    ).fetchone()[0]
    infra_error_count = c.execute(
        "SELECT COUNT(*) FROM hourly_runs WHERE infra_error != '' AND sim_hour BETWEEN ? AND ?",
        (start_hour, end_hour - 1),
    ).fetchone()[0]
    ...
    score_rows = c.execute(
        """
        SELECT model_type, output_json, anomaly, exp_lift, pricing_anomaly
        FROM hourly_runs
    ...
    for model_type, output_json, anomaly, exp_lift, pricing_anomaly in score_rows:
        try:
        ...
        score_buckets.setdefault((model_family, scenario_band), []).append((out, pricing_anomaly or "", exp_lift or ""))
    ...
    summary = {
        ...
        "infra_error_rate":       round((infra_error_count / total_runs * 100), 1) if total_runs else 0.0,
...
def main():
    ...
    for hour in range(resume_hour, TOTAL_HOURS):
        ...
        def _insert(model_type, hotel, out_json, rec_price, demand_state, conf, lift, anomalies):
            pricing_issues, infra_issues = _split_issue_channels(anomalies)
            conn.execute(
                """
                ...
                (
                    ...
                    "; ".join(pricing_issues + infra_issues),
            ...
            if pricing_issues or infra_issues:
                for a in pricing_issues + infra_issues:
        ...
        def _insert_err(model_type, hotel, exc):
            ...
            pricing_issues, infra_issues = _split_issue_channels([msg])
            conn.execute(
                """
                INSERT INTO hourly_runs (
                    run_at, sim_hour, hotel_id, hotel_name, model_type,
